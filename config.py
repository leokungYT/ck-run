...
#   1.0 = เต็ม (แม่นสุด, หนักสุด)
#   0.5-0.75 = แนะนำเมื่อรันหลายจอ (เร็วขึ้น ~2-4 เท่า แม่นน้อยลงนิดหน่อย)
SCREENCAP_SCALE = 1.0
# เว้นระยะจับภาพขั้นต่ำต่อจอ (วินาที) — ตัวนี้สำคัญสุดเมื่อรันหลายจอ!
# 16 จอยิง screencap ผ่าน adb server ตัวเดียว → จับถี่ = adb แน่น = ค้าง/หน่วง
# จับห่างขึ้น = adb โล่ง = ลื่นขึ้นทั้งระบบ (ตอบสนองช้าลงนิดเดียว เกมรอเป็นวินาทีอยู่แล้ว)
#   8-10 จอ → 0.3-0.4   |   16 จอ → 0.5-0.7   |   24+ จอ → 0.8-1.0
MIN_SCREENCAP_INTERVAL = 0.5
